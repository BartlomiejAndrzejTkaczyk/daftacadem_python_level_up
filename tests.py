...
@pytest.mark.parametrize("password", ["abcd", "hgdaa", "hvsu", "jhuhujvds"])
def test_auth_hash(password):
    password_hash = sha512(password)
    params = {"password":password, "password_hash":password_hash}
...
def test_auth_hash_predefin():
    ...
    assert sha512(password) == password_hash
